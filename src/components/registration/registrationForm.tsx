...
export default function RegistrationForm() {
      ...
      <div className="w-[500px] h-content bg-white flex flex-col rounded-[16px]">
        <div className="flex flex-col p-[15px] space-y-[16px] shadow-2xl rounded-xl">
          ...
          <input
            ...
            className={`inputStyle ${isEmptyField(error.name)}`}
            placeholder="Артур"
            type="text"
            name="name"
            value={formData.name}
          ...
          <input
            onChange={handleFormChange}
            className={`inputStyle ${error.email && "inputError"} ${isEmptyField(error.email)}`}
          ...
          <input
            onChange={handleFormChange}
            className={`inputStyle ${isEmptyField(error.password)}`}
            placeholder="****"
            type="password"
            name="password"
          ...
          <input
            onChange={handleFormChange}
            className={`inputStyle ${error.password && "inputError"} ${isEmptyField(error.confirmPassword)}`}
            placeholder="****"
            type="password"
            name="confirmPassword"
            value={formData.confirmPassword}
          />
          <button onClick={submitForm} className="buttonStyle">
            Зарегистрироваться
          </button>
        </div>
      </div>
  ...
  function checkFormNotEmpty(errorCopy: errorType) {
    if (formData.name === "") errorCopy.name = "empty";
    if (formData.password === "") errorCopy.password = "empty";
    if (formData.email === "") errorCopy.email = "empty";
    if (formData.confirmPassword === "") errorCopy.confirmPassword = "empty";
    setError(errorCopy);
  }
